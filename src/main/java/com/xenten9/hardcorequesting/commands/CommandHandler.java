...
public class CommandHandler extends CommandBase {
    ...
    public List addTabCompletionOptions(ICommandSender sender, String[] args) {
        if (args.length == 1) {
            ...
            for (ISubCommand command : commands.values()) {
                if (command.isVisible(sender) && command.getCommandName()
                    .startsWith(subCommand)) result.add(command.getCommandName());
            }
            return result;
        } else if (commands.containsKey(args[0]) && commands.get(args[0])
            .isVisible(sender)) {
                return commands.get(args[0])
                    .addTabCompletionOptions(sender, Arrays.copyOfRange(args, 1, args.length));
            }
        return null;
    }
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if (command != null) {
            if (command.isVisible(sender) && (sender.canCommandSenderUseCommand(
