...
public class CommandHardcore extends CommandBase {
    ...
    public void handleCommand(ICommandSender sender, String[] arguments) {
        if (arguments.length == 1 && arguments[0].equalsIgnoreCase("disable")) {
            ...
            if (MinecraftServer.getServer()
                .getEntityWorld()
                .getWorldInfo()
                .isHardcoreModeEnabled()) sendChat(sender, "hqm.message.vanillaHardcoreOn");
            else sendChat(
                sender,
                QuestingData.isHardcoreActive() ? "hqm.message.hardcoreAlreadyActivated" : "hqm.message.questHardcore");
            QuestingData.activateHardcore();
            if (sender instanceof EntityPlayer) currentLives((EntityPlayer) sender);
        }
    }
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args) {
        ...
        if (args[0].isEmpty() || args[0].startsWith("e")) list.add("enable");
        if (args[0].isEmpty() || args[0].startsWith("d")) list.add("disable");
        return list;
    }
}
