...
public class CommandLoad extends CommandBase {
    ...
    private static final FileFilter QUEST_SET_FILTER = new FileFilter() {
        ...
        public boolean accept(File pathname) {
            return JSON.matcher(pathname.getName())
                .find()
                && !REPUTATIONS.matcher(pathname.getName())
                    .find()
                && !BAGS.matcher(pathname.getName())
                    .find();
        }
    };
    ...
    private void loadSet(ICommandSender sender, File file) {
        ...
        try {
            if (sender instanceof EntityPlayer) HardcoreQuesting.setPlayer((EntityPlayer) sender);
            JsonReader reader = new JsonReader(new FileReader(file));
            QuestSet set = GSON.fromJson(reader, QuestSet.class);
            reader.close();
            if (set != null) {
                sender.addChatMessage(
                    new ChatComponentText(StatCollector.translateToLocalFormatted(Lang.LOAD_SUCCESS, set.getName())));
            } else {
    ...
    private void loadReputation(ICommandSender sender, File file) {
        ...
        try {
            if (sender instanceof EntityPlayer) HardcoreQuesting.setPlayer((EntityPlayer) sender);
            JsonReader reader = new JsonReader(new FileReader(file));
            List<Reputation> reputations = GSON.fromJson(
                reader,
                ...
                if (reputation != null) {
                    ...
                    sender.addChatMessage(
                        new ChatComponentText(
                            StatCollector
                                .translateToLocalFormatted(Lang.LOAD_SUCCESS, "Reputation: " + reputation.getName())));
                } else {
                    throw new CommandException(Lang.LOAD_FAILED);
                }
            }
        ...
        try {
            ...
            if (bags != null) {
                ...
                GroupTier.getTiers()
                    .addAll(bags);
                sender.addChatMessage(
                    new ChatComponentText(StatCollector.translateToLocalFormatted(Lang.LOAD_SUCCESS, "Bags")));
            } else {
                throw new CommandException(Lang.LOAD_FAILED);
            }
        } catch (Exception e) {
