...
import com.xenten9.hardcorequesting.quests.QuestTaskMob;
...
public class GuiEditMenuMob extends GuiEditMenuExtended {
    ...
    public GuiEditMenuMob(GuiQuestBook gui, QuestTaskMob task, final QuestTaskMob.Mob mob, int id,
        EntityPlayer player) {
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        for (int i = start; i < end; i++) {
            ...
            gui.drawString(
                mobs.get(i),
                START_X,
                START_Y + OFFSET_Y * (i - start),
                0.7F,
                selected ? inBounds ? 0xC0C0C0 : 0xA0A0A0 : inBounds ? 0x707070 : 0x404040);
        }

        gui.drawString(Translator.translate("hqm.mobTask.search"), 180, 20, 0x404040);
        gui.drawString(
            Translator.translate("hqm.mobTask." + (mob.getMob() == null ? "nothing" : "currently") + "Selected"),
            180,
            40,
            0x404040);
        if (mob.getMob() != null) {
    ...
    protected void save(GuiBase gui) {
        ...
        if ((mob.getIcon() == null || mob.getIcon()
            .getItem() == Items.spawn_egg) && mob.getMob() != null) {
