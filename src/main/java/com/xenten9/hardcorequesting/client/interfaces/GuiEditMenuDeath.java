...
import com.xenten9.hardcorequesting.DeathStats;
import com.xenten9.hardcorequesting.DeathType;
...
public class GuiEditMenuDeath extends GuiEditMenu {
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        int start = scrollBar.isVisible(gui) ? Math.round((deathStats.length - VISIBLE_PLAYERS) * scrollBar.getScroll())
            : 0;
        int end = Math.min(deathStats.length, start + VISIBLE_PLAYERS);
        for (int i = start; i < end; i++) {
            DeathStats stats = deathStats[i];

            boolean selected = stats.getName()
                .equals(selectedName);
            boolean inBounds = gui.inBounds(PLAYERS_X, PLAYERS_Y + (i - start) * PLAYERS_SPACING, 130, 9, mX, mY);
            gui.drawString(
                (i + 1) + ". " + stats.getName(),
                PLAYERS_X,
                PLAYERS_Y + (i - start) * PLAYERS_SPACING,
                getColor(selected, inBounds));
            String deaths = String.valueOf(stats.getTotalDeaths());
            gui.drawString(
                deaths,
                DEATHS_RIGHT - gui.getStringWidth(deaths),
                PLAYERS_Y + (i - start) * PLAYERS_SPACING,
                0x404040);
        }

        gui.drawString(
            Translator.translate(BEST_LABEL),
            BEST_X,
            LABEL_Y,
            getColor(showBest, gui.inBounds(BEST_X, LABEL_Y, gui.getStringWidth(BEST_LABEL), 9, mX, mY)));
        gui.drawString(
            Translator.translate(TOTAL_LABEL),
            TOTAL_X,
            LABEL_Y,
            getColor(showTotal, gui.inBounds(TOTAL_X, LABEL_Y, gui.getStringWidth(TOTAL_LABEL), 9, mX, mY)));
        ...
        if (stats != null) {
            ...
            for (int i = 0; i < DeathType.values().length; i++) {
                ...
                gui.drawRect(
                    TYPE_LOCATION_X + TYPE_SPACING_X * x,
                    TYPE_LOCATION_Y + TYPE_SPACING_Y * y,
                    BACKGROUND_SRC_X,
                    BACKGROUND_SRC_Y,
                    BACKGROUND_SIZE,
                    BACKGROUND_SIZE);
                gui.drawRect(
                    TYPE_LOCATION_X + TYPE_SPACING_X * x + ICON_OFFSET,
                    TYPE_LOCATION_Y + TYPE_SPACING_Y * y + ICON_OFFSET,
                    ICON_SRC_X + ICON_SIZE * x,
                    ICON_SRC_Y + ICON_SIZE * y,
                    ICON_SIZE,
                    ICON_SIZE);
            }
            ...
            gui.drawString(
                Translator.translate("hqm.deathMenu.total", stats.getTotalDeaths()),
                PLAYER_INFO_X,
                PLAYER_INFO_Y + PLAYER_TOTAL_DEATHS_Y,
                0.7F,
                0x404040);
            ...
            for (int i = 0; i < DeathType.values().length; i++) {
                ...
                gui.drawString(
                    str,
                    TYPE_LOCATION_X + TYPE_SPACING_X * x + TEXT_OFFSET_X,
                    TYPE_LOCATION_Y + TYPE_SPACING_Y * y + TEXT_OFFSET_Y + offset,
                    f,
                    0x404040);
            }
    ...
    public void onClick(GuiBase gui, int mX, int mY, int b) {
        ...
        if (gui.inBounds(BEST_X, LABEL_Y, gui.getStringWidth(Translator.translate(BEST_LABEL)), 9, mX, mY)) {
            ...
            DeathStats[] deathStats = DeathStats.getDeathStats();
            int start = scrollBar.isVisible(gui)
                ? Math.round((deathStats.length - VISIBLE_PLAYERS) * scrollBar.getScroll())
                : 0;
            int end = Math.min(deathStats.length, start + VISIBLE_PLAYERS);
            for (int i = start; i < end; i++) {
                DeathStats stats = deathStats[i];
