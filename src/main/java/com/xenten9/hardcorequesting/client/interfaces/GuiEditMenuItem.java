...
public class GuiEditMenuItem extends GuiEditMenu {
    ...
    public GuiEditMenuItem(GuiBase gui, EntityPlayer player, Object obj, int id, Type type, int amount,
        ItemPrecision precision) {
        this(
            gui,
            player,
            obj instanceof ItemStack ? new ElementItem((ItemStack) obj) : new ElementFluid((Fluid) obj),
            id,
            type,
            amount,
            precision);
    ...
    public GuiEditMenuItem(GuiBase gui, EntityPlayer player, Element element, int id, final Type type, final int amount,
        ItemPrecision precision) {
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        if (usePrecision()) {
            ...
            gui.drawCenteredString(
                precision.getName(),
                ARROW_X_LEFT + ARROW_W,
                ARROW_Y,
                0.7F,
                ARROW_X_RIGHT - (ARROW_X_LEFT + ARROW_W),
                ARROW_H,
                0x404040);
        }
        ...
        if (usePrecision()) {
            if (inArrowBounds(gui, mX, mY, true)) {
                ...
                precision = precisionTypes
                    .get((precisionTypes.indexOf(precision) + precisionTypes.size() - 1) % precisionTypes.size());
                clicked = true;
    ...
    protected void save(GuiBase gui) {
        if (type == Type.BAG_ITEM) {
            if (GuiQuestBook.getSelectedGroup() != null && selected instanceof ElementItem
                && (ItemStack) selected.getItem() != null) {
                GuiQuestBook.getSelectedGroup()
                    .setItem(id, (ItemStack) selected.getItem());
            }
        } else if (type == Type.QUEST_ICON) {
            if (Quest.getQuest(id) != null && selected instanceof ElementItem) {
                try {
                    Quest.getQuest(id)
                        .setIcon((ItemStack) selected.getItem());
                } catch (Exception e) {
    ...
    public static class Search implements Runnable {
            ...
            if (searchItems.isEmpty()) {
                ...
                for (Object anItemRegistry : Item.itemRegistry) {
                    try {
                        ...
                        // if (itemBlock.field_150939_a == Blocks.lava || itemBlock.field_150939_a == Blocks.water ||
                        // itemBlock.field_150939_a instanceof BlockLiquid || itemBlock.field_150939_a instanceof
                        // IFluidBlock)
                        // continue;
                        // }
        ...
        public static class SearchEntry {
            ...
            public void search(Pattern pattern, List<GuiEditMenuItem.Element> stackList, boolean advanced) {
                if (pattern.matcher(advanced ? advToolTip : toolTip)
                    .find()) {
