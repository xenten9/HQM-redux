...
public abstract class GuiEditMenuExtended extends GuiEditMenu {
    ...
    protected GuiEditMenuExtended(GuiBase gui, EntityPlayer player, boolean isControlOnFirstPage, int arrowX,
        int arrowY, int boxX, int boxY) {
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        if (isArrowVisible()) {
            ...
            if (description != null) {
                gui.drawString(
                    gui.getLinesFromText(description, 0.7F, ARROW_X_RIGHT - ARROW_X_LEFT + ARROW_W),
                    ARROW_X_LEFT,
                    ARROW_DESCRIPTION_Y,
                    0.7F,
                    0x404040);
