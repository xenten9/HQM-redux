...
import com.xenten9.hardcorequesting.quests.QuestTaskLocation;
...
public class GuiEditMenuLocation extends GuiEditMenuExtended {
    ...
    public GuiEditMenuLocation(GuiQuestBook gui, QuestTaskLocation task, final QuestTaskLocation.Location location,
        ...
        textBoxes.add(new TextBoxNumberNegative(gui, 4, "hqm.locationMenu.radius") {
            ...
            protected void draw(GuiBase gui, boolean selected) {
                ...
                gui.drawString(
                    gui.getLinesFromText(Translator.translate("hqm.locationMenu.negRadius"), 0.7F, 130),
                    BOX_X,
                    BOX_Y + BOX_OFFSET * 5 + TEXT_OFFSET,
                    0.7F,
                    0x404040);
            }
        });
    ...
    protected void onArrowClick(boolean left) {
        if (left) {
            location.setVisible(
                QuestTaskLocation.Visibility.values()[(location.getVisible()
                    .ordinal() + QuestTaskLocation.Visibility.values().length
                    - 1) % QuestTaskLocation.Visibility.values().length]);
        } else {
            location.setVisible(
                QuestTaskLocation.Visibility.values()[(location.getVisible()
                    .ordinal() + 1) % QuestTaskLocation.Visibility.values().length]);
        }
    }

    @Override
    protected String getArrowText() {
        return location.getVisible()
            .getName();
    }
