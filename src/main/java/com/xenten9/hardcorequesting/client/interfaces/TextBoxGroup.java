    ...
    public static class TextBox extends TextBoxLogic {
        ...
        protected void draw(GuiBase gui, boolean selected) {
            ...
            gui.drawRect(
                ...
                y,
                TEXT_BOX_SRC_X,
                TEXT_BOX_SRC_Y + (selected ? TEXT_BOX_HEIGHT : 0),
                TEXT_BOX_WIDTH,
                TEXT_BOX_HEIGHT);
        ...
        protected void recalculateCursor(GuiBase gui) {
            if (scrollable) {
                if (updatedCursor) {
                    updateVisible(gui);
                    cursorPositionX = (int) (getMult()
                        * gui.getStringWidth(visibleText.substring(0, Math.min(visibleText.length(), cursor - start))));
                    updatedCursor = false;
                }
            } else {
