...
public class GuiEditMenuTier extends GuiEditMenu {
    ...
    public GuiEditMenuTier(GuiQuestBook gui, EntityPlayer player, final GroupTier tier) {
        ...
        for (int i = 0; i < values.length; i++) {
            final int id = i;
            textBoxes.add(
                new TextBoxGroup.TextBox(
                    gui,
                    String.valueOf(tier.getWeights()[id]),
                    TIERS_WEIGHTS_X + TIERS_TEXT_BOX_X,
                    TIERS_WEIGHTS_Y + TIERS_WEIGHTS_SPACING * id + TIERS_TEXT_BOX_Y,
                    false) {
    ...
    public void onClick(GuiBase gui, int mX, int mY, int b) {
        ...
        if (inArrowBounds(gui, mX, mY, true)) {
            tier.setColor(
                GuiColor.values()[(tier.getColor()
                    .ordinal() + GuiColor.values().length
                    - 1) % GuiColor.values().length]);
            clicked = true;
        } else if (inArrowBounds(gui, mX, mY, false)) {
            tier.setColor(
                GuiColor.values()[(tier.getColor()
                    .ordinal() + 1) % GuiColor.values().length]);
            clicked = true;
        }
