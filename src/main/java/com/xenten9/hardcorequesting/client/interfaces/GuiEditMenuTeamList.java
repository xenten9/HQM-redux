...
import com.xenten9.hardcorequesting.TeamStats;
...
public class GuiEditMenuTeamList extends GuiEditMenu {
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        for (int i = start; i < end; i++) {
            ...
            gui.drawString(
                Translator.translate("hqm.teamList.done", teamStat.getProgress()),
                x + TEAM_LINE_INDENT,
                y + TEAM_LINE_OFFSET,
                0.7F,
                0x404040);
            gui.drawString(
                Translator.translate("hqm.teamList.players", teamStat.getPlayers()),
                x + TEAM_LINE_INDENT,
                y + TEAM_LINE_OFFSET * 2,
                0.7F,
                0x404040);
            gui.drawString(
                Translator.translate("hqm.teamList.lives", teamStat.getLives()),
                x + TEAM_LINE_INDENT,
                y + TEAM_LINE_OFFSET * 3,
                0.7F,
                0x404040);
        }

        gui.drawCenteredString(
            Translator.translate("hqm.teamList.page", ((pagePair * 2) + 1)),
            0,
            202,
            0.7F,
            170,
        ...
        if (end - start > TEAMS_PER_PAGE) {
            gui.drawCenteredString(
                Translator.translate("hqm.teamList.page", ((pagePair * 2) + 2)),
                170,
                202,
                0.7F,
                170,
    ...
    private void drawArrow(GuiBase gui, int mX, int mY, boolean left) {
        ...
        gui.drawRect(
            ...
            ARROW_Y,
            ARROW_SRC_X,
            ARROW_SRC_Y + srcY * ARROW_W,
            ARROW_H,
            ARROW_W,
            left ? RenderRotation.ROTATE_90 : RenderRotation.ROTATE_270);
    }

    private boolean isArrowEnabled(boolean left) {
        return (left && pagePair > 0)
            || (!left && pagePair < Math.ceil((float) TeamStats.getTeamStats().length / TEAMS_PER_PAIR) - 1);
    }
