...
import com.xenten9.hardcorequesting.client.EditButton;
import com.xenten9.hardcorequesting.client.EditMode;
import com.xenten9.hardcorequesting.client.KeyboardHandler;
...
import com.xenten9.hardcorequesting.network.FileHelper;
import com.xenten9.hardcorequesting.network.PacketHandler;
import com.xenten9.hardcorequesting.quests.*;
...
public class GuiQuestBook extends GuiBase {
...

    {
        ...
        scrollBars
            .add(reputationTierScroll = new ScrollBar(312, 23, 186, 171, 69, Reputation.REPUTATION_MARKER_LIST_X) {
        ...
        scrollBars.add(reputationScroll = new ScrollBar(160, 23, 186, 171, 69, Reputation.REPUTATION_LIST_X) {
            ...
            public boolean isVisible(GuiBase gui) {
                return isReputationPage && !isBagPage
                    && (getCurrentMode() != EditMode.CREATE || selectedReputation == null)
...

    {
        ...
        buttons.add(new LargeButton("hqm.questBook.open", 245, 190) {
            ...
            @Override
            public boolean isVisible(GuiBase gui, EntityPlayer player) {
                return editMenu == null && selectedSet != null
                    && !isBagPage
                    && !isSetOpened
                    && !isMainPageOpen
                    && !isMenuPageOpen
        ...
        buttons.add(new LargeButton("hqm.questBook.createSet", 185, 50) {
            ...
            @Override
            public boolean isVisible(GuiBase gui, EntityPlayer player) {
                return editMenu == null && Quest.isEditing
        ...
        buttons.add(new LargeButton("hqm.questBook.createGroup", 100, 175) {
            ...
            @Override
            public boolean isVisible(GuiBase gui, EntityPlayer player) {
                return editMenu == null && isBagPage
                    && currentMode == EditMode.CREATE
                    && selectedGroup == null
                    && !isMainPageOpen
                    && !isMenuPageOpen
    ...
    public void drawScreen(int x0, int y0, float f) {
        ...
        if (shouldDisplayControlArrow(false)) {
            drawRect(
                BACK_ARROW_X,
                BACK_ARROW_Y,
                BACK_ARROW_SRC_X + (inArrowBounds(false, x, y) ? BACK_ARROW_WIDTH : 0),
                BACK_ARROW_SRC_Y,
                BACK_ARROW_WIDTH,
                BACK_ARROW_HEIGHT);
        }
        if (shouldDisplayControlArrow(true)) {
            drawRect(
                MENU_ARROW_X,
                MENU_ARROW_Y,
                MENU_ARROW_SRC_X + (inArrowBounds(true, x, y) ? MENU_ARROW_WIDTH : 0),
                MENU_ARROW_SRC_Y,
                MENU_ARROW_WIDTH,
                MENU_ARROW_HEIGHT);
        }
        ...
        if (Quest.isEditing) {
            if (saveResult != null) {
                if (saveButton.inButtonBounds(this, x, y) || SaveHelper.inSaveBounds(this, x, y)) {
                    String str = (saveResult == FileHelper.SaveResult.SUCCESS ? GuiColor.GREEN : GuiColor.RED)
                        .toString();
    ...
    private void drawMenuPage(int x, int y) {
        ...
        if (QuestingData.isHardcoreActive()) {
            boolean almostOut = QuestingData.getQuestingData(player)
                .getLives()
            ...
            int lives = QuestingData.getQuestingData(player)
                .getLives();
            ...
            drawString(
                getLinesFromText(Translator.translate("hqm.questBook.infiniteLives"), 0.5F, PAGE_WIDTH - 30),
                INFO_RIGHT_X,
                INFO_LIVES_Y + 12,
                0.5F,
                0x707070);
        ...
        int deaths = DeathStats.getDeathStats(QuestingData.getUserName(player))
            .getTotalDeaths();
        drawString(
        ...
        Team team = QuestingData.getQuestingData(player)
            .getTeam();
        if (team.isSingle()) {
            int invites = team.getInvites() == null ? 0
                : team.getInvites()
                    .size();
            if (invites > 0) {
        ...
        if (isOpBook) {
            ...
            drawString(
                getLinesFromText(Translator.translate("hqm.questBook.shiftCtrlConfirm"), 0.6F, 70),
                22,
                192,
                0.6F,
    ...
    private void drawMainPage() {
        int startLine = mainDescriptionScroll.isVisible(this) ? Math.round(
            (Quest.getMainDescription(this)
                .size() - VISIBLE_MAIN_DESCRIPTION_LINES) * mainDescriptionScroll.getScroll())
            : 0;
        drawString(
            Quest.getMainDescription(this),
            startLine,
    ...
    protected void mouseClicked(int x0, int y0, int button) {
        ...
        for (LargeButton largeButton : buttons) {
            if (largeButton.isVisible(this, player) && largeButton.isEnabled(this, player)
    ...
    private EditButton[] getButtons() {
        return isMainPageOpen ? mainButtons
            : isMenuPageOpen ? menuButtons
                : isReputationPage ? reputationButtons
                    : isBagPage ? selectedGroup != null ? groupButtons : bagButtons
                        : selectedSet == null || !isSetOpened ? overviewButtons
                            : selectedQuest == null ? setButtons : questButtons;
    }

    private EditButton[] groupButtons = EditButton.createButtons(this, EditMode.NORMAL, EditMode.ITEM, EditMode.DELETE);
    private EditButton[] bagButtons = EditButton
        .createButtons(this, EditMode.NORMAL, EditMode.CREATE, EditMode.RENAME, EditMode.TIER, EditMode.DELETE);
    private EditButton[] reputationButtons = EditButton.createButtons(
        this,
        EditMode.NORMAL,
        EditMode.CREATE,
        EditMode.RENAME,
    ...
    private EditButton[] setButtons = EditButton.createButtons(
        ...
        EditMode.DELETE);
    private EditButton[] questButtons = EditButton.createButtons(
        this,
        EditMode.NORMAL,
        EditMode.RENAME,
        EditMode.TASK,
    ...
    private boolean shouldDisplayControlArrow(boolean isMenuArrow) {
        return !isMainPageOpen
            && ((!(isMenuArrow && isMenuPageOpen) && editMenu == null) || (editMenu != null && !editMenu.hasButtons()));
    }
