...
public class GuiEditMenuPortal extends GuiEditMenuExtended {
    ...
    public GuiEditMenuPortal(GuiBase gui, EntityPlayer player, TileEntityPortal portal) {
        ...
        checkboxes
            .add(new CheckBox("hqm.portalMenu.collisionNonComplete", CHECK_BOX_X, CHECK_BOX_Y + CHECK_BOX_OFFSET * 2) {
        ...
        checkboxes
            .add(new CheckBox("Use textures when not completed", CHECK_BOX_X, CHECK_BOX_Y + CHECK_BOX_OFFSET * 3) {
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        gui.drawCenteredString(
            portal.getCurrentQuest() != null ? portal.getCurrentQuest()
                .getName() : Translator.translate("hqm.portalMenu.noQuest"),
            0,
            5,
            1F,
            170,
    ...
    protected void onArrowClick(boolean left) {
        if (left) {
            portal.setType(
                PortalType.values()[(portal.getType()
                    .ordinal() + PortalType.values().length
                    - 1) % PortalType.values().length]);
        } else {
            portal.setType(
                PortalType.values()[(portal.getType()
                    .ordinal() + 1) % PortalType.values().length]);
        }
    }
