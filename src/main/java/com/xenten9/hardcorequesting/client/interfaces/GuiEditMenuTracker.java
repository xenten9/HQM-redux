...
import com.xenten9.hardcorequesting.tileentity.TrackerType;
...
public class GuiEditMenuTracker extends GuiEditMenuExtended {
    ...
    public GuiEditMenuTracker(GuiBase gui, EntityPlayer player, final TileEntityTracker tracker) {
        ...
        textBoxes.add(new TextBoxNumber(gui, 0, "hqm.menuTracker.radius.title") {
            ...
            protected void draw(GuiBase gui, boolean selected) {
                ...
                gui.drawString(
                    gui.getLinesFromText(Translator.translate("hqm.menuTracker.radius.desc"), 0.7F, 130),
                    BOX_X,
                    BOX_Y + BOX_OFFSET + TEXT_OFFSET,
                    0.7F,
                    0x404040);
            }
        });
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        gui.drawCenteredString(
            tracker.getCurrentQuest() != null ? tracker.getCurrentQuest()
                .getName() : Translator.translate("hqm.menuTracker.noQuest"),
            0,
            5,
            1F,
            170,
    ...
    protected void onArrowClick(boolean left) {
        if (left) {
            tracker.setType(
                TrackerType.values()[(tracker.getType()
                    .ordinal() + TrackerType.values().length
                    - 1) % TrackerType.values().length]);
        } else {
            tracker.setType(
                TrackerType.values()[(tracker.getType()
                    .ordinal() + 1) % TrackerType.values().length]);
        }
    }
