...
public class GuiEditMenuReputationReward extends GuiEditMenuExtended {
    ...
    public GuiEditMenuReputationReward(GuiBase gui, EntityPlayer player, List<ReputationReward> rewards) {
        ...
        buttons.add(new LargeButton("hqm.repReward.create", 20, 20) {
            ...
            public void onClick(GuiBase gui, EntityPlayer player) {
                GuiEditMenuReputationReward.this.rewards.add(
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        if (isValid()) {
            for (int i = 0; i < rewards.size(); i++) {
                String str = rewards.get(i)
                    .getLabel();
                boolean hover = gui.inBounds(START_X, START_Y + i * OFFSET, gui.getStringWidth(str), 9, mX, mY);
                boolean selected = rewards.get(i)
                    .equals(selectedReward);
                gui.drawString(
                    str,
                    START_X,
                    START_Y + i * OFFSET,
                    selected ? hover ? 0x40CC40 : 0x409040 : hover ? 0xAAAAAA : 0x404040);
            }
        } else {
            if (error == null) {
    ...
    private boolean isValid() {
        return !Reputation.getReputationList()
            .isEmpty();
    }
    ...
    protected void onArrowClick(boolean left) {
        if (selectedReward != null && selectedReward.getReward() != null) {
            for (int i = 0; i < Reputation.getReputationList()
                .size(); i++) {
                if (Reputation.getReputationList()
                    .get(i)
                    .equals(selectedReward.getReward())) {
                    int id = i + (left ? -1 : 1);
                    if (id < 0) {
                        id = Reputation.getReputationList()
                            .size() - 1;
                    } else if (id >= Reputation.getReputationList()
                        .size()) {
                            id = 0;
                        }
    ...
    protected String getArrowText() {
        return selectedReward.getReward()
            .getName();
    }
