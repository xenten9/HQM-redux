...
public class GuiEditMenuTeam extends GuiEditMenu {
    ...
    public GuiEditMenuTeam(GuiQuestBook gui, EntityPlayer player) {
        ...
        buttons.add(new LargeButton(null, 250, 50) {
            ...
            protected String getName() {
                return Translator
                    .translate(selectedEntry.isInTeam() ? "hqm.party.kickPlayer" : "hqm.party.removeInvite");
            }
        });
        ...
        scrollBars.add(inviteScroll = new ScrollBar(155, 22, 186, 171, 69, PLAYER_X) {
            ...
            public boolean isVisible(GuiBase gui) {
                return inviteTeam == null && getTeam().isSingle()
                    && getTeam().getInvites() != null
                    && getTeam().getInvites()
                        .size() > VISIBLE_INVITES;
            }
        });
        ...
        scrollBars.add(memberScroll = new ScrollBar(155, 22, 186, 171, 69, PLAYER_X) {
            ...
            @Override
            public boolean isVisible(GuiBase gui) {
                return (inviteTeam != null && inviteTeam.getPlayers()
                    .size() > VISIBLE_MEMBERS) || (!getTeam().isSingle()
                        && getTeam().getPlayers()
                            .size() > VISIBLE_MEMBERS);
            }
        });
    }
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        if (team.isSingle() && inviteTeam == null) {
            int inviteCount = team.getInvites() == null ? 0
                : team.getInvites()
                    .size();
            if (inviteCount > 0) {
                gui.drawString(Translator.translate("hqm.party.invites"), TITLE_X, TITLE_Y, 0x404040);
                List<Team> invites = team.getInvites();
                int start = inviteScroll.isVisible(gui) ? Math.round(
                    (team.getInvites()
                        .size() - VISIBLE_INVITES) * inviteScroll.getScroll())
                    : 0;
            ...
            int y = 0;
            int start = memberScroll.isVisible(gui) ? Math.round(
                ((isOwner ? players.size() : (inviteTeam == null ? team : inviteTeam).getPlayerCount())
                    - VISIBLE_MEMBERS) * memberScroll.getScroll())
                : 0;
                ...
                if (y >= start) {
                    ...
                    if (isOwner) {
                        if (player.equals(selectedEntry)) {
                            ...
                            PLAYER_X,
                            PLAYER_Y + PLAYER_SPACING * (y - start),
                            (int) (gui.getStringWidth(player.getName()) * 0.7F),
            ...
            if (inviteTeam == null) {
                if (entry.isOwner()) {
                    ...
                    if (selectedEntry != null) {
                        gui.drawString(
                            gui.getLinesFromText(
                                Translator.translate("hqm.party.currentSelection", selectedEntry.getName()),
                                0.7F,
                                70),
                            177,
                            52,
                        ...
                        if (selectedEntry.isOwner()) {
                            gui.drawString(
                                gui.getLinesFromText(Translator.translate("hqm.party.shiftCtrlConfirm"), 0.6F, 70),
                                177,
                                162,
                                0.6F,
                    ...
                    gui.drawString(
                        gui.getLinesFromText(Translator.translate("hqm.party.shiftConfirm"), 0.7F, 70),
                        177,
                        162,
                        0.7F,
                        GuiColor.RED.getHexColor());
            ...
            gui.drawString(
                gui.getLinesFromText(Translator.translate("hqm.party.stats"), 0.7F, 70),
                177,
                192,
                0.7F,
                0x404040);
            ...
            gui.drawString(
                Translator.translate(
                    "hqm.party.lifeSetting",
                    infoTeam.getLifeSetting()
                        .getTitle()),
                INFO_BOX_X + INFO_BOX_TEXT_OFFSET_X,
                infoY + INFO_BOX_TEXT_OFFSET_Y,
                0.7F,
                0x404040);
            gui.drawString(
                Translator.translate(
                    "hqm.party.rewardSetting",
                    infoTeam.getRewardSetting()
                        .getTitle()),
                INFO_BOX_X + INFO_BOX_TEXT_OFFSET_X,
                infoY + REWARD_SETTING_Y + INFO_BOX_TEXT_OFFSET_Y,
                0.7F,
                0x404040);
    ...
    public void drawMouseOver(GuiBase gui, int mX, int mY) {
        ...
        if (!team.isSingle() || inviteTeam != null) {
            ...
            if (gui.inBounds(INFO_BOX_X, infoY, INFO_BOX_SIZE, INFO_BOX_SIZE, mX, mY)) {
                gui.drawMouseOver(
                    gui.getLinesFromText(
                        GuiColor.GREEN + infoTeam.getLifeSetting()
                            .getTitle()
                            + "\n"
                            + infoTeam.getLifeSetting()
                                .getDescription()
                            + (isOwner ? "\n\n" + GuiColor.ORANGE + Translator.translate("hqm.party.change") : ""),
                        1F,
                        200),
                    gui.getLeft() + mX,
                    gui.getTop() + mY);
                ...
                gui.drawMouseOver(
                    gui.getLinesFromText(
                        GuiColor.GREEN + infoTeam.getRewardSetting()
                            .getTitle()
                            + "\n"
                            + infoTeam.getRewardSetting()
                                .getDescription()
        ...
        if (Team.latestError != null) {
            if (inviteButton.inButtonBounds(gui, mX, mY)) {
                gui.drawMouseOver(
                    gui.getLinesFromText(
                        GuiColor.RED + Team.latestError.getHeader() + "\n" + Team.latestError.getMessage(),
                        1F,
                        150),
                    mX + gui.getLeft(),
                    mY + gui.getTop());
        ...
        if (team.isSingle() && inviteTeam == null) {
            ...
            if (invites != null) {
                ...
                for (int i = start; i < end; i++) {
                    ...
                    if (gui.inBounds(
                        PLAYER_X,
                        PLAYER_Y + PLAYER_SPACING * i,
                        (int) (gui.getStringWidth(invite.getName()) * 0.7F),
                        (int) (TEXT_HEIGHT * 0.7F),
                        mX,
                        mY)) {
                        inviteTeam = invite;
            ...
            int start = memberScroll.isVisible(gui) ? Math.round(
                (team.getPlayers()
                    .size() - VISIBLE_MEMBERS) * memberScroll.getScroll())
                : 0;
            int end = Math.min(
                team.getPlayers()
                    .size(),
                start + VISIBLE_MEMBERS);
            for (int i = start; i < end; i++) {
                Team.PlayerEntry entry = team.getPlayers()
                    .get(i);
                if (gui.inBounds(
                    PLAYER_X,
                    PLAYER_Y + PLAYER_SPACING * (i - start),
                    (int) (gui.getStringWidth(entry.getName()) * 0.7F),
                    (int) (TEXT_HEIGHT * 0.7F),
                    mX,
                    mY)) {
