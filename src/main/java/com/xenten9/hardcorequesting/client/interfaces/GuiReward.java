...
public class GuiReward extends GuiBase {
    ...
    public GuiReward(Group group, int bagTier, EntityPlayer player) {
        ...
        for (Group other : Group.getGroups()) {
            if (other.isValid(player)) {
                totalWeight += other.getTier()
                    .getWeights()[bagTier];
            }
        }

        int myWeight = group.getTier()
            .getWeights()[bagTier];
        ...
        lines = (int) Math.ceil(
            (float) group.getItems()
                .size() / ITEMS_PER_LINE);
        for (int i = 0; i < lines; i++) {
            int y = TOP_HEIGHT + MIDDLE_HEIGHT * i + (MIDDLE_HEIGHT - ITEM_SIZE) / 2;
            int itemsInLine = Math.min(
                group.getItems()
                    .size() - i * ITEMS_PER_LINE,
                ITEMS_PER_LINE);
            for (int j = 0; j < itemsInLine; j++) {
                int x = (TEXTURE_WIDTH - (itemsInLine * ITEM_SIZE + (itemsInLine - 1) * ITEM_MARGIN)) / 2
                    + j * (ITEM_SIZE + ITEM_MARGIN);
                ItemStack item = group.getItems()
                    .get(i * ITEMS_PER_LINE + j);
    ...
    public void drawScreen(int mX0, int mY0, float f) {
        ...
        drawCenteredString(
            Translator.translate("hqm.rewardGui.close"),
            0,
            TOP_HEIGHT + lines * MIDDLE_HEIGHT,
            0.7F,
            TEXTURE_WIDTH,
            BOTTOM_HEIGHT,
            0x707070);
            ...
            if (inBounds(reward.x, reward.y, ITEM_SIZE, ITEM_SIZE, mX, mY)) {
                try {
                    if (GuiScreen.isShiftKeyDown()) {
                        ...
                        try {
                            List info = reward.item.getTooltip(
                                Minecraft.getMinecraft().thePlayer,
                                Minecraft.getMinecraft().gameSettings.advancedItemTooltips);
                            if (info.size() > 0) {
    ...
    public static void open(EntityPlayer player, DataReader dr) {
        Group rewardGroup = Group.getGroups()
            .get(dr.readData(DataBitHelper.GROUP_COUNT));
        ...
        if (ItemBag.displayGui) {
            Minecraft.getMinecraft()
                .displayGuiScreen(new GuiReward(rewardGroup, bagTier, player));
        }
    }
