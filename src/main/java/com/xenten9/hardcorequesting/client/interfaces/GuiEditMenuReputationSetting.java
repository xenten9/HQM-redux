...
public class GuiEditMenuReputationSetting extends GuiEditMenuExtended {
    ...
    public GuiEditMenuReputationSetting(GuiQuestBook gui, EntityPlayer player, QuestTaskReputation task, int id,
        QuestTaskReputation.ReputationSetting setting) {
        super(gui, player, true, 25, 25, -1, -1);

        this.task = task;
        this.id = id;
        if (setting == null || setting.getReputation() == null) {
            if (!Reputation.getReputationList()
                .isEmpty()) {
                reputation = Reputation.getReputationList()
                    .get(0);
                reputationId = 0;
            } else {
                reputationId = -1;
    ...
    public void draw(GuiBase gui, int mX, int mY) {
        ...
        if (reputation != null) {
            ...
            info = reputation.draw(
                (GuiQuestBook) gui,
                BARS_X,
                LOWER_Y + BAR_OFFSET_Y,
                mX,
                mY,
                info,
                player,
                ...
                false,
                lower,
                lower == null ? "" : "Selected: " + lower.getLabel(),
                false);
            ...
            info = reputation.draw(
                ...
                false,
                upper,
                upper == null ? "" : "Selected: " + upper.getLabel(),
                false);
    ...
    public void onClick(GuiBase gui, int mX, int mY, int b) {
        ...
        if (reputation != null) {
            ReputationMarker marker = reputation
                .onActiveClick((GuiQuestBook) gui, BARS_X, LOWER_Y + BAR_OFFSET_Y, mX, mY);
            if (marker != null) {
    ...
    protected void onArrowClick(boolean left) {
        if (reputation != null) {
            ...
            if (reputationId < 0) {
                reputationId = Reputation.getReputationList()
                    .size() - 1;
            } else if (reputationId >= Reputation.getReputationList()
                .size()) {
                    reputationId = 0;
                }
            lower = null;
