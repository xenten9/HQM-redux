...
import com.xenten9.hardcorequesting.client.interfaces.GuiEditMenuReputationValue;
import com.xenten9.hardcorequesting.client.interfaces.GuiEditMenuTextEditor;
import com.xenten9.hardcorequesting.client.interfaces.GuiQuestBook;
import com.xenten9.hardcorequesting.client.interfaces.ResourceHelper;
...
import com.xenten9.hardcorequesting.quests.QuestTaskReputation;
import com.xenten9.hardcorequesting.reward.ReputationReward;
...
public class Reputation {
    ...
    public static void drawAll(GuiQuestBook gui, int x, int y, int mX, int mY, final EntityPlayer player) {
        ...
        Collections.sort(reputations, new Comparator<Reputation>() {
            ...
            public int compare(Reputation reputation1, Reputation reputation2) {
                return ((Integer) Math.abs(reputation2.getValue(player)))
                    .compareTo(Math.abs(reputation1.getValue(player)));
            }
        });

        int start = gui.reputationDisplayScroll.isVisible(gui) ? Math.round(
            (reputations.size() - GuiQuestBook.VISIBLE_DISPLAY_REPUTATIONS) * gui.reputationDisplayScroll.getScroll())
            : 0;
        int end = Math.min(start + GuiQuestBook.VISIBLE_DISPLAY_REPUTATIONS, reputations.size());
        for (int i = start; i < end; i++) {
            gui.applyColor(0xFFFFFFFF);
            ResourceHelper.bindResource(GuiQuestBook.MAP_TEXTURE);
            info = reputations.get(i)
                .draw(
                    gui,
                    x,
                    y + (i - start) * OFFSET_Y,
    ...
    @SideOnly(Side.CLIENT)
    public String draw(GuiQuestBook gui, int x, int y, int mX, int mY, String info, EntityPlayer player,
        boolean effects, ReputationMarker lower, ReputationMarker upper, boolean inverted, ReputationMarker active,
        ...
        for (int i = 0; i < markers.size(); i++) {
            ...
            boolean selected = markers.get(i)
                .equals(active) || (effects && ((lowerValue <= value && value <= upperValue) != inverted));
        ...
        if (drawPointer(
            ...
            mX,
            mY,
            neutral.equals(active) || (effects && ((lowerValue <= 0 && 0 <= upperValue) != inverted)))) {
        ...
        if (text != null) {
            ...
            if (lower == null && upper == null) {
                str = GuiColor.RED + Translator.translate("hqm.rep" + (inverted ? "no" : "any") + "ValueOf")
                    + " "
                    + name;

            } else {
                String lowerName = lower == null ? null
                    : GuiScreen.isShiftKeyDown() ? String.valueOf(lower.getValue()) : lower.getName();
                String upperName = upper == null ? null
                    : GuiScreen.isShiftKeyDown() ? String.valueOf(upper.getValue()) : upper.getName();
                    ...
                    if (lower.equals(upper)) {
                        ...
                        if (inverted) {
                            str = Translator.translate(
                                "hqm.rep.not") + " (" + lowerName + " <= " + name + " <= " + upperName + ")";
                        } else {
            ...
            selected = completed
                || (effects && ((lowerValue <= current.getValue() && current.getValue() <= upperValue) != inverted));
        }
    ...
    private boolean drawPointer(GuiQuestBook gui, int value, int x, int y, int offsetY, int srcX, int mX, int mY,
        boolean selectedTexture) {
        boolean flag = false;
        ...
        gui.drawRect(
            ...
            pointerY,
            srcX,
            ARROW_SRC_Y + (selectedTexture ? -ARROW_SIZE : 0),
            ARROW_SIZE,
            ARROW_SIZE);
    ...
    private int getPointerPosition(int value, boolean onMarker) {
        ...
        for (int i = 0; i < markers.size(); i++) {
            ...
            if (value <= marker.getValue()) {
                ...
                if (pointAtMarker) {
                    pointerPosition = ARROW_MARKER_OFFSET
                        + i * (BAR_WIDTH - ARROW_MARKER_OFFSET * 2) / (markers.size() - 1);
                    ...
                    pointerPosition = ARROW_MARKER_OFFSET
                        + (int) ((i - 1 + ((float) internalValue / span)) * (BAR_WIDTH - ARROW_MARKER_OFFSET * 2)
                            / (markers.size() - 1));
                }
    ...
    public static void drawEditPage(GuiQuestBook gui, int mX, int mY) {
        if (gui.getCurrentMode() != EditMode.CREATE || selectedReputation == null) {
            int start = gui.reputationScroll.isVisible(gui)
                ? Math.round(
                    (reputationList.size() - GuiQuestBook.VISIBLE_REPUTATIONS) * gui.reputationScroll.getScroll())
                : 0;
            ...
            for (int i = start; i < end; i++) {
                ...
                boolean selected = reputationList.get(i)
                    .equals(selectedReputation);
        ...
        if (selectedReputation != null) {
            ...
            gui.drawString(
                neutralName,
                REPUTATION_MARKER_LIST_X,
                REPUTATION_NEUTRAL_Y,
                gui.inBounds(
                    REPUTATION_MARKER_LIST_X,
                    REPUTATION_NEUTRAL_Y,
                    gui.getStringWidth(neutralName),
                    FONT_HEIGHT,
                    mX,
                    mY) ? 0xAAAAAA : 0x404040);

            int start = gui.reputationTierScroll.isVisible(gui) ? Math.round(
                (selectedReputation.markers.size() - GuiQuestBook.VISIBLE_REPUTATION_TIERS)
                    * gui.reputationTierScroll.getScroll())
                : 0;
            ...
            for (int i = start; i < end; i++) {
                ...
                String str = selectedReputation.markers.get(i)
                    .getTitle();
                            ...
                            for (QuestTask task : quest.getTasks()) {
                                if (task instanceof QuestTaskReputation) {
                                    ...
                                    for (QuestTaskReputation.ReputationSetting setting : reputationTask.getSettings()) {
                                        if (selectedReputation.markers.get(i)
                                            .equals(setting.getLower())) {
                                            setting.setLower(null);
                                        }
                                        if (selectedReputation.markers.get(i)
                                            .equals(setting.getUpper())) {
        ...
        if (version.contains(FileVersion.REPUTATION)) {
            ...
            for (int i = 0; i < reputationCount; i++) {
                ...
                for (int j = 0; j < markerCount; j++) {
                    ReputationMarker marker = new ReputationMarker(
                        dr.readString(DataBitHelper.QUEST_NAME_LENGTH),
