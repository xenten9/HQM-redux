...
import com.xenten9.hardcorequesting.client.interfaces.GuiEditMenu;
...
public class ReputationBar {
    ...
    public boolean isValid() {
        return Quest.getQuestSets()
            .size() > this.questSet && getQuestSet() != null
            && Reputation.getReputation(this.repId) != null;
    }
    ...
    public void draw(GuiQuestBook gui, int mX, int mY, EntityPlayer player) {
        ...
        String info = reputation
            .draw(gui, this.x, this.y, mX, mY, null, player, false, null, null, false, null, null, false);
    ...
    public boolean inBounds(int mX, int mY) {
        return this.x <= mX && this.x + Reputation.BAR_WIDTH >= mX
    ...
    public static class EditGui extends GuiEditMenu {
        ...
        public void draw(GuiBase guiB, int mX, int mY) {
            ...
            int start = gui.reputationScroll.isVisible(gui) ? Math.round(
                (Reputation.getReputationList()
                    .size() - GuiQuestBook.VISIBLE_REPUTATIONS) * gui.reputationScroll.getScroll())
                : 0;
            int end = Math.min(
                start + GuiQuestBook.VISIBLE_REPUTATIONS,
                Reputation.getReputationList()
                    .size());
            for (int i = start; i < end; i++) {
                int x = Reputation.REPUTATION_LIST_X;
                int y = Reputation.REPUTATION_LIST_Y + (i - start) * Reputation.REPUTATION_OFFSET;
                String str = Reputation.getReputationList()
                    .get(i)
                    .getName();

                boolean hover = gui.inBounds(x, y, gui.getStringWidth(str), Reputation.FONT_HEIGHT, mX, mY);
                boolean selected = Reputation.getReputationList()
                    .get(i)
                    .equals(Reputation.getReputation(bar.repId));

                gui.drawString(str, x, y, selected ? hover ? 0x40CC40 : 0x409040 : hover ? 0xAAAAAA : 0x404040);
            }
            gui.drawString(
                gui.getLinesFromText(Translator.translate("hqm.rep.select"), 1F, 120),
                Reputation.REPUTATION_MARKER_LIST_X,
                Reputation.REPUTATION_LIST_Y,
                1F,
                0x404040);
        }
        ...
        public void onClick(GuiBase guiB, int mX, int mY, int b) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (gui.inBounds(x, y, gui.getStringWidth(str), Reputation.FONT_HEIGHT, mX, mY)) {
                    bar.repId = Reputation.getReputationList()
                        .get(i)
                        .getId();
                    save(guiB);
                    close(guiB);
