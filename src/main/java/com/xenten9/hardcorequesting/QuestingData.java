...
import com.xenten9.hardcorequesting.bag.GroupData;
...
public class QuestingData {
    ...
    public void removeLifeAndSendMessage(EntityPlayer player) {
        ...
        if (!isDead) {
            player.addChatMessage(
                new ChatComponentText(Translator.translate(getLives() != 1, "hqm.message.lostLife", getLives())));
        }
        if (getTeam().isSharingLives()) {
            for (Team.PlayerEntry entry : getTeam().getPlayers()) {
                if (entry.isInTeam() && !entry.getName()
                    .equals(getUserName(player))) {
                        ...
                        other.addChatMessage(
                            new ChatComponentText(
                                Translator.translate(
                                    getLives() != 1,
                                    "hqm.message.lostTeamLife",
                                    getUserName(player),
                                    (isDead ? " " + Translator.translate("hqm.message.andBanned") : ""),
                                    getLives())));
                    }
                }
            }
    ...
    public boolean removeLives(EntityPlayer player, int amount) {
        ...
        if (shareLives) {
            ...
            while (amount > 0) {
                int players = 0;
                for (Team.PlayerEntry entry : QuestingData.getQuestingData(player)
                    .getTeam()
                    .getPlayers()) {
                    if (!entry.getName()
                        .equals(getUserName(player))
    ...
    private void outOfLives(EntityPlayer playerEntity) {
        ...
        playerEntity.inventory.clearInventory(null, -1); // had some problem with tconstruct, clear all items to prevent
                                                         // it

        MinecraftServer mcServer = MinecraftServer.getServer();

        if (mcServer.isSinglePlayer() && playerEntity.getCommandSenderName()
            .equals(mcServer.getServerOwner())) {
             ...
             * mcServer.getActiveAnvilConverter().deleteWorldDirectory(mcServer.worldServers[0].getSaveHandler().
    ...
    public static void activateHardcore() {
        if (!hardcoreActive && !MinecraftServer.getServer()
            .getEntityWorld()
            .getWorldInfo()
     ...
     * lives (no quest book) and call that file version 0. Then when we want to save the quest book we increase
     * FILE_VERSION
     * to 1 and start developing. When it's released it's released as file version 1 even though the quest book's saving
     * might have changed a few times during the development. The important things is that version 0 is still the old
     * thing
     * without the quest book so it won't try to load the quest book otherwise.
     * Files from newer versions are obviously not going to be able to be read. So if the player is still running a
     * version
    ...
    private QuestingData(FileVersion version, DataReader dr) {
        ...
        for (Object teammateObj : team.getPlayers()) {
            ...
            if (QuestingData.data.containsKey(name)) {
                ...
                if (teammatePlayer.getTeam()
                    .getId() != team.getId()) QuestingData.data.remove(name);
            }
        }
    }
        ...
        if (isQuestActive()) {
            ...
            if (!light) {
                ...
                for (int i = 0; i < groupData.size(); i++) {
                    if (Group.getGroup(i) != null && Group.getGroup(i)
                        .getLimit() > 0) {
    ...
    public void sendDataToClientAndOpenInterface(EntityPlayer player, String name) {
        EventHandler.instance()
            .onEvent(
                new EventHandler.BookOpeningEvent(
    ...
    public static void spawnBook(EntityPlayer player) {
        if (!Quest.isEditing && !player.worldObj.isRemote
            && ModConfig.spawnBook
            && !QuestingData.getQuestingData(player).receivedBook
            && QuestingData.isQuestActive()) {
    ...
    private static void spawnItemAtPlayer(EntityPlayer player, ItemStack stack) {
        ...
        if (!(player instanceof FakePlayer)) item.onCollideWithPlayer(player);
    }
    ...
    public static boolean hasData(String playerName) {
        if (data.containsKey(playerName)) {
            ...
            return player != null && player.getGameProfile()
                .getName()
                .equals(playerName);
        }
    }
