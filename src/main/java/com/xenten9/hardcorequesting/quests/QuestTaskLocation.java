...
public class QuestTaskLocation extends QuestTask {
            ...
            if (!world.isRemote) {
                ...
                for (int i = 0; i < locations.length; ++i) {
                    ...
                    if (!visited[i] && player.worldObj.provider.dimensionId == location.dimension) {
                        int current = (int) player.getDistanceSq(
                            (double) location.x + 0.5D,
                            (double) location.y + 0.5D,
                            (double) location.z + 0.5D);
                        int target = location.radius * location.radius;
                        if (location.radius >= 0 && current > target) {
                            all = false;
                        } else {
                            if (!this.isCompleted(player) && this.isVisible(player)
                                && this.parent.isEnabled(player)
                                && this.parent.isAvailable(player)) {
                                updated = true;
                                visited[i] = true;
                            }
                        }
    ...
    public void draw(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
        ...
        for (int i = 0; i < locations.length; i++) {
            ...
            if (visited(i, player)) {
                gui.drawString(
                    GuiColor.GREEN + Translator.translate("hqm.locationMenu.visited"),
                    x + X_TEXT_OFFSET + X_TEXT_INDENT,
                    y + Y_TEXT_OFFSET + 9,
                    0.7F,
                    0x404040);
                ...
                if (player.worldObj.provider.dimensionId == location.dimension) {
                    ...
                    gui.drawString(
                        Translator.translate("hqm.locationMenu.wrongDim"),
                        x + X_TEXT_OFFSET + X_TEXT_INDENT,
                        y + Y_TEXT_OFFSET + (location.radius >= 0 ? 15 : 9),
                        0.7F,
                        0x404040);
                }
