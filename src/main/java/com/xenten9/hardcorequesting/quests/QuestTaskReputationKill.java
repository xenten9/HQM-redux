...
public class QuestTaskReputationKill extends QuestTaskReputation {
    ...
    public void onLivingDeath(LivingDeathEvent event) {
        ...
        if (killer != null && parent.isEnabled(killer)
            && parent.isAvailable(killer)
            && this.isVisible(killer)
            && !this.isCompleted(killer)
            && !killer.equals(event.entityLiving)) {
    ...
    public void draw(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
        ...
        if (Quest.isEditing) {
            gui.drawString(
                gui.getLinesFromText(Translator.translate(kills != 1, "hqm.repKil.kills", killCount, kills), 1F, 130),
                START_X,
                START_Y,
                1F,
                0x404040);
            ...
            gui.drawString(
                gui.getLinesFromText(
                    killCount == kills
                        ? GuiColor.GREEN + Translator.translate(kills != 1, "hqm.repKil.killCount", kills)
                        : Translator.translate("hqm.repKil.killCountOutOf", killCount, kills),
                    1F,
                    130),
                START_X,
                START_Y,
    ...
    public void mergeProgress(String playerName, QuestDataTask own, QuestDataTask other) {
        ((QuestDataTaskReputationKill) own).kills = Math
            .max(((QuestDataTaskReputationKill) own).kills, ((QuestDataTaskReputationKill) other).kills);
