...
import com.xenten9.hardcorequesting.client.interfaces.GuiEditMenuReputationSetting;
...
public abstract class QuestTaskReputation extends QuestTask {
    // for this task to be completed, all reputation settings (up to 4) has to be completed at the same time, therefore
    // it's not saved whether you've completed one of these reputation settings, just if you've completed it all
    ...
    public static class ReputationSetting {
        ...
        public boolean isValid(String playerName) {
            ...
            return ((lower == null || lower.getValue() <= current.getValue())
                && (upper == null || current.getValue() <= upper.getValue())) != inverted;
        }
    ...
    public void load(DataReader dr, FileVersion version) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ReputationMarker lower = dr.readBoolean()
                ? reputation.getMarker(dr.readData(DataBitHelper.REPUTATION_MARKER))
                : null;
            ReputationMarker upper = dr.readBoolean()
                ? reputation.getMarker(dr.readData(DataBitHelper.REPUTATION_MARKER))
                : null;
    ...
    public void draw(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
        String info = null;
        int size = Quest.isEditing ? Math.min(settings.length + 1, DataBitHelper.REPUTATION_SETTING.getMaximum())
            : settings.length;
        for (int i = 0; i < size; i++) {
            ...
            if (i >= settings.length) {
                gui.drawRect(
                    START_X + Reputation.BAR_X,
                    START_Y + startOffsetY + i * OFFSET_Y + Reputation.BAR_Y,
                    Reputation.BAR_SRC_X,
                    Reputation.BAR_SRC_Y,
                    Reputation.BAR_WIDTH,
                    Reputation.BAR_HEIGHT);
            } else {
                ReputationSetting setting = settings[i];
                info = setting.reputation.draw(
                    gui,
                    START_X,
                    START_Y + startOffsetY + i * OFFSET_Y,
                    mX,
                    mY,
                    info,
                    getPlayerForRender(player),
                    true,
                    setting.lower,
                    setting.upper,
                    setting.inverted,
            ...
            for (int i = 0; i < size; i++) {
                if (gui.inBounds(START_X, START_Y + startOffsetY + i * OFFSET_Y, Reputation.BAR_WIDTH, 20, mX, mY)) {
                    if (gui.getCurrentMode() == EditMode.REPUTATION_TASK) {
                        gui.setEditMenu(
                            new GuiEditMenuReputationSetting(
                                gui,
                                player,
                                this,
                                i,
                                i >= settings.length ? null : settings[i]));
