...
public abstract class QuestTaskItems extends QuestTask {
    ...
    public static class ItemRequirement {
        ...
        public ItemStack getPermutatedItem() {
            if (permutations == null && precision.hasPermutations()) setPermutations();
            if (permutations == null || permutations.length < 2) return item;
            int ticks = (int) (System.currentTimeMillis() / 1000);
            if (cycleAt == -1) cycleAt = ticks + CYCLE_TIME;
            if (ticks >= cycleAt) {
                if (++current > last) current = 0;
                while (ticks >= cycleAt) cycleAt += CYCLE_TIME;
            }
    ...
    public void draw(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            float textSize = 0.8F;
            gui.drawStringWithShadow(
                str,
                (int) (item.x + SIZE - gui.getStringWidth(str) * textSize),
                (int) (item.y + SIZE - TEXT_HEIGHT * textSize + 2),
                textSize,
                getProgress(player, i) == item.required ? 0x308030 : 0xFFFFFF);
            GL11.glPopMatrix();
            // gui.setZLevel(z);
        }
                    ...
                    if (isOpBookWithShiftKeyDown) {
                        ...
                        gui.setEditMenu(
                            new GuiEditMenuItem(
                                gui,
                                player,
                                item.hasItem ? item.item != null ? item.item.copy() : null : item.fluid,
                                i,
                                getMenuTypeId(),
                                item.required,
                                item.precision));
                    } else if (Quest.isEditing && gui.getCurrentMode() == EditMode.DELETE
                        && (item.item != null || item.fluid != null)) {
