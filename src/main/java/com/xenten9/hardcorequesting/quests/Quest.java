...
import com.xenten9.hardcorequesting.client.EditMode;
...
import com.xenten9.hardcorequesting.reputation.ReputationBar;
import com.xenten9.hardcorequesting.reward.*;
...
public class Quest {
...

    {
        ...
        buttons.add(new LargeButton("hqm.quest.manualSubmit", 185, 200) {
            ...
            @Override
            public boolean isVisible(GuiBase gui, EntityPlayer player) {
                return selectedTask != null && selectedTask instanceof QuestTaskItemsConsume
                    && !selectedTask.isCompleted(player);
            }
        ...
        buttons.add(new LargeButton("hqm.quest.manualDetect", 185, 200) {
            ...
            @Override
            public boolean isVisible(GuiBase gui, EntityPlayer player) {
                return selectedTask != null && selectedTask instanceof QuestTaskItemsDetect
                    && !selectedTask.isCompleted(player);
            }
        ...
        buttons.add(new LargeButton("hqm.quest.selectTask", 250, 200) {
            ...
            @Override
            public boolean isEnabled(GuiBase gui, EntityPlayer player) {
                return QuestingData.getQuestingData(player).selectedQuest != getId()
            ...
            @Override
            public void onClick(GuiBase gui, EntityPlayer player) {
                // update locally too, then we don't have to refresh all the data(i.e. the server won't notify us about
                // the change we already know about)
        ...
        for (final TaskType taskType : TaskType.values()) {
            buttons.add(
                new LargeButton(
                    taskType.getLangKeyName(),
                    taskType.getLangKeyDescription(),
                    185 + (taskType.ordinal() % 2) * 65,
                    50 + (taskType.ordinal() / 2) * 35) {
                    ...
                    @Override
                    public boolean isVisible(GuiBase gui, EntityPlayer player) {
                        return isEditing && selectedTask == null
                            && ((GuiQuestBook) gui).getCurrentMode() == EditMode.TASK;
                    }
            ...
            if (QuestTaskItems.class.isAssignableFrom(taskType.clazz)) {
                buttons.add(
                    new LargeButton(
                        ...
                        public boolean isVisible(GuiBase gui, EntityPlayer player) {
                            return isEditing && selectedTask != null
                                && ((GuiQuestBook) gui).getCurrentMode() == EditMode.CHANGE_TASK;
                        }
                        ...
                        public void onClick(GuiBase gui, EntityPlayer player) {
                            ...
                            try {
                                Constructor<? extends QuestTask> constructor = clazz
                                    .getConstructor(Quest.class, String.class, String.class);
                                QuestTask task = constructor
                                    .newInstance(self, taskType.getLangKeyName(), taskType.getLangKeyDescription());
                                ...
                                if (!selectedTask.getLangKeyDescription()
                                    .equals(oldTaskType.getLangKeyName())) {
                                    task.setDescription(selectedTask.getLangKeyDescription());
                                }
                                if (!selectedTask.getLangKeyLongDescription()
                                    .equals(oldTaskType.getLangKeyDescription())) {
...

    {
        ...
        scrollBars.add(taskDescriptionScroll = new ScrollBar(312, 18, 64, 249, 102, TASK_DESCRIPTION_X) {
            ...
            @Override
            public boolean isVisible(GuiBase gui) {
                return selectedTask != null && selectedTask.getCachedLongDescription(gui)
    ...
    public static List<String> getMainDescription(GuiBase gui) {
        if (QuestLine.getActiveQuestLine().cachedMainDescription == null) {
            QuestLine.getActiveQuestLine().cachedMainDescription = gui
                .getLinesFromText(QuestLine.getActiveQuestLine().mainDescription, 0.7F, 130);
        }
    ...
    public QuestData getQuestData(EntityPlayer player) {
        return QuestingData.getQuestingData(player)
            .getQuestData(id);
    }

    public QuestData getQuestData(String playerName) {
        return QuestingData.getQuestingData(playerName)
            .getQuestData(id);
    }
    ...
    boolean isVisible(String playerName, Map<Quest, Boolean> isVisibleCache, Map<Quest, Boolean> isLinkFreeCache) {
        ...
        boolean result = triggerType.isQuestVisible(this, playerName) && isLinkFree(playerName, isLinkFreeCache)
            && visibleParentEvaluator.isValid(playerName, isVisibleCache, isLinkFreeCache);
    ...
    boolean isEnabled(String playerName, boolean requiresVisible, Map<Quest, Boolean> isVisibleCache,
        Map<Quest, Boolean> isLinkFreeCache) {
        return !(set == null || !isLinkFree(playerName, isLinkFreeCache)
            || (requiresVisible && !triggerType.doesWorkAsInvisible()
                && !isVisible(playerName, isVisibleCache, isLinkFreeCache)))
            && enabledParentEvaluator.isValid(playerName, isVisibleCache, isLinkFreeCache);
    }
    ...
    private ParentEvaluator enabledParentEvaluator = new ParentEvaluator() {
        ...
        @Override
        protected boolean isValid(String playerName, Quest parent, Map<Quest, Boolean> isVisibleCache,
            Map<Quest, Boolean> isLinkFreeCache) {
    ...
    private abstract class ParentEvaluator {
        ...
        protected abstract boolean isValid(String playerName, Quest parent, Map<Quest, Boolean> isVisibleCache,
            Map<Quest, Boolean> isLinkFreeCache);

        private boolean isValid(String playerName, Map<Quest, Boolean> isVisibleCache,
            Map<Quest, Boolean> isLinkFreeCache) {
    ...
    public void drawMenu(GuiQuestBook gui, EntityPlayer player, int mX, int mY) {
        ...
        int startLine = descriptionScroll.isVisible(gui)
            ? Math.round((getCachedDescription(gui).size() - VISIBLE_DESCRIPTION_LINES) * descriptionScroll.getScroll())
            : 0;
        gui.drawString(
            getCachedDescription(gui),
            startLine,
            VISIBLE_DESCRIPTION_LINES,
            START_X,
            DESCRIPTION_START_Y,
            0.7F,
            0x404040);
        ...
        int id = 0;
        int start = taskScroll.isVisible(gui)
            ? Math.round((getVisibleTasks(gui) - VISIBLE_TASKS) * taskScroll.getScroll())
            : 0;
        ...
        for (int i = start; i < end; i++) {
            ...
            if (isVisible || Quest.isEditing) {
                ...
                boolean inBounds = gui
                    .inBounds(START_X, yPos, gui.getStringWidth(task.getDescription()), TEXT_HEIGHT, mX, mY);
                boolean isSelected = task == selectedTask;
                gui.drawString(
                    task.getDescription(),
                    START_X,
                    yPos,
                    completed ? isSelected ? inBounds ? 0x40BB40 : 0x40A040 : inBounds ? 0x10A010 : 0x107010
                        : isSelected ? inBounds ? 0xAAAAAA : 0x888888
                            : inBounds ? 0x666666 : isVisible ? 0x404040 : 0xDDDDDD);
        ...
        if (!rewards.isEmpty() || isEditing) {
            ...
            if (!rewardChoices.isEmpty() || isEditing) {
                gui.drawString(
                    Translator.translate("hqm.quest.pickOne"),
                    START_X,
                    REWARD_STR_Y + REWARD_Y_OFFSET,
                    0x404040);
                drawRewards(
                    gui,
                    rewardChoices.toArray(),
                    REWARD_Y + REWARD_Y_OFFSET,
                    selectedReward,
                    mX,
                    mY,
                    MAX_REWARD_SLOTS);
        ...
        int y = rewards == null || rewards.size() <= MAX_REWARD_SLOTS - (isEditing ? 2 : 1) ? REPUTATION_Y_LOWER
            : REPUTATION_Y;
        ...
        if (reputationRewards != null || isEditing) {
            ...
            gui.drawRect(
                REPUTATION_X,
                y,
                REPUTATION_SRC_X + backgroundIndex * REPUTATION_SIZE,
                REPUTATION_SRC_Y,
                REPUTATION_SIZE,
                REPUTATION_SIZE);
            gui.drawRect(
                REPUTATION_X,
                y,
                REPUTATION_SRC_X + foregroundIndex * REPUTATION_SIZE,
                REPUTATION_SRC_Y,
                REPUTATION_SIZE,
                REPUTATION_SIZE);
        }
        ...
        if (selectedTask != null) {
            if (isEditing && gui.getCurrentMode() == EditMode.CHANGE_TASK) {
                if (selectedTask instanceof QuestTaskItems) {
                    gui.drawString(
                        gui.getLinesFromText(Translator.translate("hqm.quest.itemTaskChangeTo"), 0.7F, 130),
                        180,
                        20,
                        0.7F,
                        0x404040);
                    ...
                    gui.drawString(
                        gui.getLinesFromText(Translator.translate("hqm.quest.itemTaskTypeOnly"), 0.7F, 130),
                        180,
                        20,
                        0.7F,
                        0x404040);
                ...
                int taskStartLine = taskDescriptionScroll.isVisible(gui)
                    ? Math.round((description.size() - VISIBLE_DESCRIPTION_LINES) * taskDescriptionScroll.getScroll())
                    : 0;
                gui.drawString(
                    description,
                    taskStartLine,
            ...
            gui.drawString(
                gui.getLinesFromText(Translator.translate("hqm.quest.createTasks"), 0.7F, 130),
                180,
                20,
                0.7F,
                0x404040);
            ...
            gui.drawString(
                gui.getLinesFromText(Translator.translate("hqm.quest.itemTaskTypeChange"), 0.7F, 130),
                180,
                20,
                0.7F,
                0x404040);
        ...
        if (reputationRewards != null && hover) {
            ...
            for (ReputationReward reputationReward : reputationRewards) {
                if (reputationReward.getValue() != 0 && reputationReward.getReward() != null
                    && reputationReward.getReward()
                        .isValid()) {
            ...
            List<String> commentLines = gui.getLinesFromText(
                Translator.translate("hqm.quest.partyRepReward" + (claimed ? "Claimed" : "")),
                1,
                200);
    ...
    public boolean hasReward(EntityPlayer player) {
        return (getQuestData(player).getReward(player) && (!rewards.isEmpty() || !rewardChoices.isEmpty()))
            || (getQuestData(player).canClaim() && (reputationRewards != null || !commandRewardList.isEmpty()));
    }
    ...
    @SideOnly(Side.CLIENT)
    private void handleRewardClick(GuiQuestBook gui, EntityPlayer player, ItemStack[] rawRewards, int y,
        boolean canSelect, int mX, int mY) {
        ItemStack[] rewards = getEditFriendlyRewards(
            rawRewards,
            canSelect ? MAX_SELECT_REWARD_SLOTS : MAX_REWARD_SLOTS);
                ...
                if (canSelect && (!isEditing || gui.getCurrentMode() == EditMode.NORMAL)) {
                    ...
                    gui.setEditMenu(
                        new GuiEditMenuItem(
                            ...
                            rewards[i],
                            i,
                            canSelect ? GuiEditMenuItem.Type.PICK_REWARD : GuiEditMenuItem.Type.REWARD,
                            rewards[i] == null ? 1 : rewards[i].stackSize,
    ...
    public void onClick(GuiQuestBook gui, EntityPlayer player, int mX, int mY, int b) {
        if (b == 1) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (task.isVisible(player) || isEditing) {
                    ...
                    if (gui.inBounds(
                        START_X,
                        getTaskY(gui, id),
                        gui.getStringWidth(task.getDescription()),
                        TEXT_HEIGHT,
                        mX,
                        mY)) {
            ...
            for (LargeButton button : buttons) {
                if (button.inButtonBounds(gui, mX, mY) && button.isVisible(gui, player)
                    && button.isEnabled(gui, player)) {
            ...
            if (gui.getCurrentMode() == EditMode.RENAME) {
                ...
                } else if (gui.inBounds(
                    START_X,
                    DESCRIPTION_START_Y,
                    130,
                    (int) (VISIBLE_DESCRIPTION_LINES * TEXT_HEIGHT * 0.7),
                    mX,
                    mY)) {
                        gui.setEditMenu(new GuiEditMenuTextEditor(gui, player, this, false));
                    } else if (selectedTask != null && gui.inBounds(
                        TASK_DESCRIPTION_X,
                        TASK_DESCRIPTION_Y,
                        130,
                        (int) (VISIBLE_DESCRIPTION_LINES * TEXT_HEIGHT * 0.7),
    ...
    public boolean addTaskData(QuestData data) {
        ...
        for (int i = 0; i < tasks.size(); i++) {
            try {
                Constructor constructor = tasks.get(i)
                    .getDataType()
                    .getConstructor(QuestTask.class);
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (i >= tasks.size()) {
                    try {
                        ...
                        nextTaskId--; // we created a fake task, it shouldn't occupy an id. It doesn't really matter
                                      // since we're not editing, but what ever.

                        Constructor<? extends QuestDataTask> constructor2 = task.getDataType()
                            .getConstructor(new Class[] { QuestTask.class });
                        Object obj2 = constructor2.newInstance(task);

                        task.read(dr, (QuestDataTask) obj2, version, false); // read data to clear the data reader from
                                                                             // it
                        task.onDelete();
                    } catch (Exception ex) {
                        ex.printStackTrace();
    ...
    public void sendUpdatedDataToTeam(String playerName) {
        sendUpdatedDataToTeam(
            QuestingData.getQuestingData(playerName)
                .getTeam());
    }
    ...
    public void claimReward(EntityPlayer player, DataReader dr) {
        if (hasReward(player)) {
                    ...
                    for (ItemStack itemStack : itemsToCheck) {
                        if (itemStack.stackSize > 0) {
                            if (player.inventory.mainInventory[i] == null) {
                                ...
                                break;
                            } else if (player.inventory.mainInventory[i].isItemEqual(itemStack)
                                && ItemStack.areItemStackTagsEqual(itemStack, player.inventory.mainInventory[i])) {
                                    itemStack.stackSize -= itemStack.getMaxStackSize()
            ...
            if (reputationRewards != null && getQuestData(player).canClaim()) {
                ...
                QuestingData.getQuestingData(player)
                    .getTeam()
                    .receiveAndSyncReputation(this, reputationRewards);
                EventHandler.instance()
                    .onEvent(new EventHandler.ReputationEvent(player));
                sentInfo = true;
            }
        ...
        for (int i = 0; i < player.inventory.mainInventory.length; i++) {
            ...
            while (iterator.hasNext()) {
                ...
                if (player.inventory.mainInventory[i] == null) {
                    ...
                    && ItemStack.areItemStackTagsEqual(itemStack, player.inventory.mainInventory[i])) {
                        int amount = Math.min(
                            itemStack.getMaxStackSize() - player.inventory.mainInventory[i].stackSize,
                            itemStack.stackSize);
    ...
    public void setItem(GuiEditMenuItem.Element element, int id, GuiEditMenuItem.Type type, ItemPrecision precision,
        EntityPlayer player) {
        ...
        } else if (selectedTask != null && selectedTask instanceof QuestTaskLocation
            && type == GuiEditMenuItem.Type.LOCATION) {
                ((QuestTaskLocation) selectedTask).setIcon(id, (ItemStack) element.getItem(), player);
            } else
            if (selectedTask != null && selectedTask instanceof QuestTaskMob && type == GuiEditMenuItem.Type.MOB) {
    ...
    public enum TaskType {
        ...
        public QuestTask addTask(Quest quest) {
            QuestTask prev = quest.getTasks()
                .size() > 0 ? quest.getTasks()
                    .get(
                        quest.getTasks()
                            .size() - 1)
                    : null;
    ...
    public static void saveAll(DataWriter dw) {
        ...
        for (int id = 0; id < size(); id++) {
            ...
            if (quest != null) {
                ...
                if (quest.getIcon() != null && quest.getIcon()
                    .getItem() != null) {
                ...
                for (int i = 0; i < quest.tasks.size(); i++) {
                    ...
                    int type = TaskType.getType(task.getClass())
                        .ordinal();
            ...
            for (ItemStack itemStack : reward) {
                if (itemStack != null && itemStack.getItem() != null) {
                    ...
                    FMLLog.log(
                        "HQM",
                        Level.ERROR,
                        "The quest %s has an invalid item reference in it's rewards - substituting with HQM invalid item",
                        quest.getName());
    ...
    private static void writeRewardData(DataWriter dw, CommandRewardList commands) {
        ...
        if (!commands.isEmpty()) {
            ...
            for (String command : commands.asStrings()) dw.writeString(command, DataBitHelper.QUEST_DESCRIPTION_LENGTH);
        }
    }
        ...
        if (dr != null) {
            ...
            try {
                if (version.lacks(FileVersion.LORE)) {
                    ...
                    QuestLine.getActiveQuestLine().mainDescription = dr
                        .readString(DataBitHelper.QUEST_DESCRIPTION_LENGTH);
                }

                if (version.lacks(FileVersion.SETS)) {
                    QuestLine.getActiveQuestLine().questSets.add(
                        new QuestSet(
                            "Automatically generated",
                            "This set was automatically generated. All your quests were put in this one."));
                } else {
                                ...
                                for (int i = 0; i < reputationCount; i++) {
                                    quest.reputationRewards.add(
                                        new ReputationReward(
                                            Reputation.getReputation(dr.readData(DataBitHelper.REPUTATION)),
                                            dr.readData(DataBitHelper.REPUTATION_VALUE)));
                                }
                            }
                        } else {
                            quest.reputationRewards = null;
