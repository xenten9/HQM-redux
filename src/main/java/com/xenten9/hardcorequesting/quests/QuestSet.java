...
import com.xenten9.hardcorequesting.OPBookHelper;
...
public class QuestSet {
    ...
    private void validateBars() {
        ...
        for (ReputationBar reputationBar : reputationBars) if (!reputationBar.isValid()) toRemove.add(reputationBar);
    ...
    public void decreaseId() {
        id--;
        for (ReputationBar reputationBar : reputationBars) reputationBar.setQuestSet(this.id);
    }
    ...
    public static void drawOverview(GuiQuestBook gui, ScrollBar setScroll, ScrollBar descriptionScroll, int x, int y) {
        ...
        int start = setScroll.isVisible(gui) ? Math.round(
            (Quest.getQuestSets()
        ...
        for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_SETS, questSets.size()); i++) {
            ...
            int total = questSet.getQuests()
                .size();
            boolean enabled = questSet.isEnabled(player, isVisibleCache, isLinkFreeCache);
            int completedCount = enabled ? questSet.getCompletedCount(player, isVisibleCache, isLinkFreeCache) : 0; // no
                                                                                                                    // need
                                                                                                                    // to
                                                                                                                    // check
                                                                                                                    // for
            ...
            boolean inBounds = gui.inBounds(
                ...
                x,
                y);
            int color = gui.modifyingQuestSet == questSet ? 0x4040DD
                : enabled
            ...
            String info;
            if (enabled) {
                if (completed) info = Translator.translate("hqm.questBook.allQuests");
                else info = Translator.translate("hqm.questBook.percentageQuests", ((completedCount * 100) / total));
            } else info = Translator.translate("hqm.questBook.locked");
            gui.drawString(info, GuiQuestBook.LIST_X + LINE_2_X, setY + LINE_2_Y, 0.7F, color);
            if (enabled && unclaimed != 0) {
                String toClaim = GuiColor.PURPLE.toString()
        ...
        if ((Quest.isEditing && gui.getCurrentMode() == EditMode.CREATE)) {
            ...
            if (GuiQuestBook.selectedSet != null) {
                int startLine = descriptionScroll.isVisible(
    ...
    public void draw(GuiQuestBook gui, int x0, int y0, int x, int y) {
                ...
                for (Quest parent : child.getRequirement()) {
                    if (Quest.isEditing || parent.isVisible(player, isVisibleCache, isLinkFreeCache)) {
                        if (parent.hasSameSetAs(child)) {
                            int color = Quest.isEditing && (!child.isVisible(player, isVisibleCache, isLinkFreeCache)
                                || !parent.isVisible(player, isVisibleCache, isLinkFreeCache)) ? 0x55404040
                                    : 0xFF404040;
                            gui.drawLine(
        ...
        if (Quest.isEditing) {
            for (Quest child : getQuests()) {
                for (Quest parent : child.getOptionLinks()) {
                    if (parent.hasSameSetAs(child)) {
                        int color = !child.isVisible(player, isVisibleCache, isLinkFreeCache)
        ...
        for (Quest quest : getQuests()) {
            if ((Quest.isEditing || quest.isVisible(player, isVisibleCache, isLinkFreeCache))) {
                ...
                // drawRect(quest.getIconX(), quest.getIconY(), quest.getIconU(), quest.getIconV(), quest.getIconSize(),
                // quest.getIconSize());
            }
        }
        ...
        for (Quest quest : getQuests()) {
            ...
            if ((editing || quest.isVisible(player, isVisibleCache, isLinkFreeCache)) && quest.isMouseInObject(x, y)) {
                ...
                if (enabled || editing) {
                    ...
                    String repeatMessage = enabled ? quest.getRepeatInfo()
                    ...
                    if (editing) {
                        ...
                        String triggerMessage = quest.getTriggerType()
                            .getMessage(quest);
    ...
    public void mouseClicked(GuiQuestBook gui, int x, int y) {
            ...
            switch (gui.getCurrentMode()) {
                case CREATE:
                    if (x > 0 && Quest.size() < DataBitHelper.QUESTS.getMaximum()) {
                        int i = 0;
                        for (Quest quest : this.getQuests()) if (quest.getName()
                            .startsWith("Unnamed")) i++;
                        ...
                        switch (gui.getCurrentMode()) {
                            ...
                            case REQUIREMENT:
                                if (gui.modifyingQuest == quest) {
                                    if (GuiScreen.isShiftKeyDown()) gui.modifyingQuest.clearRequirements();
                            ...
                            case QUEST_OPTION:
                                if (gui.modifyingQuest == quest) {
                                    if (GuiScreen.isShiftKeyDown()) gui.modifyingQuest.clearOptionLinks();
                                    gui.modifyingQuest = null;
                                } else if (gui.modifyingQuest == null) {
                                    gui.modifyingQuest = quest;
                                } else {
        ...
        if (Quest.isEditing) for (ReputationBar reputationBar : new ArrayList<ReputationBar>(this.getReputationBars()))
