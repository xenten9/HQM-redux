                    ...
                    if (te != null && te instanceof TileEntityTracker) {
                        if (!Quest.isEditing) {
                            player.addChatMessage(
                                Translator.translateToIChatComponent("tile.hqm:quest_tracker.offLimit"));;
                        } else {
                            ((TileEntityTracker) te).setCurrentQuest();
                            if (((TileEntityTracker) te).getCurrentQuest() != null) {
                                player.addChatMessage(
                                    Translator.translateToIChatComponent(
                                        "tile.hqm:quest_tracker.bindTo",
                                        ((TileEntityTracker) te).getCurrentQuest()
                                            .getName()));
                            } else {
                                player
                                    .addChatMessage(Translator.translateToIChatComponent("hqm.message.noTaskSelected"));
                            }
                        }
