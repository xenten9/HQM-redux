...
import com.xenten9.hardcorequesting.Team;
import com.xenten9.hardcorequesting.items.ItemBag;
...
public class ModConfig {
    ...
    public void onConfigChangedEvent(ConfigChangedEvent.OnConfigChangedEvent event) {
        if (event.modID.equalsIgnoreCase(ModInformation.ID)) loadConfig();
    }

    private static void loadConfig() {
        int lives = config.get(CATEGORY_GENERAL, LIVES_KEY, LIVES_DEFAULT, LIVES_COMMENT)
            .getInt(LIVES_DEFAULT);
        MAXLIVES = config.get(CATEGORY_GENERAL, MAXLIVES_KEY, MAXLIVES_DEFAULT, MAXLIVES_COMMENT)
            .getInt(MAXLIVES_DEFAULT);
        ...
        QuestingData.autoHardcoreActivate = config.get(CATEGORY_GENERAL, AUTO_KEY, AUTO_DEFAULT, AUTO_COMMENT)
            .getBoolean(AUTO_DEFAULT);

        QuestingData.autoQuestActivate = config
            .get(CATEGORY_GENERAL, AUTO_QUEST_KEY, AUTO_QUEST_DEFAULT, AUTO_QUEST_COMMENT)
            .getBoolean(AUTO_QUEST_DEFAULT);

        Team.RewardSetting.isAllModeEnabled = config
            .get(CATEGORY_GENERAL, MULTI_REWARD_KEY, MULTI_REWARD_DEFAULT, MULTI_REWARD_COMMENT)
            .getBoolean(MULTI_REWARD_DEFAULT);

        ItemBag.displayGui = config.get(CATEGORY_GENERAL, REWARD_KEY, REWARD_DEFAULT, REWARD_COMMENT)
            .getBoolean(REWARD_DEFAULT);

        spawnBook = config.get(CATEGORY_GENERAL, SPAWNBOOK_KEY, SPAWNBOOK_DEFAULT, SPAWNBOOK_COMMENT)
            .getBoolean(SPAWNBOOK_DEFAULT);

        QuestLine.doServerSync = config.get(CATEGORY_GENERAL, SYNC_KEY, SYNC_DEFAULT, SYNC_COMMENT)
            .getBoolean(SYNC_DEFAULT);

        ROTTIMER = config.get(CATEGORY_GENERAL, FRESHNESS_KEY, FRESHNESS_DEFAULT, FRESHNESS_COMMENT)
            .getBoolean(FRESHNESS_DEFAULT);
        MAXROT = config.get(CATEGORY_GENERAL, ROT_KEY, ROT_DEFAULT, ROT_COMMENT)
            .getInt(ROT_DEFAULT);

        NO_HARDCORE_MESSAGE = config
            .get(CATEGORY_GENERAL, NO_HARDCORE_MESSAGE_KEY, NO_HARDCORE_MESSAGE_DEFAULT, NO_HARDCORE_MESSAGE_COMMENT)
            .getBoolean(NO_HARDCORE_MESSAGE_DEFAULT);

        ALWAYS_USE_TIER_NAME_FOR_REWARD_TITLES = config
            .get(
                CATEGORY_GENERAL,
                ALWAYS_USE_TIER_NAME_FOR_REWARD_TITLES_KEY,
                ALWAYS_USE_TIER_NAME_FOR_REWARD_TITLES_DEFAULT,
                ALWAYS_USE_TIER_NAME_FOR_REWARD_TITLES_COMMENT)
            .getBoolean(ALWAYS_USE_TIER_NAME_FOR_REWARD_TITLES_DEFAULT);

        LOSE_QUEST_BOOK_ON_DEATH = config
            .get(
                CATEGORY_GENERAL,
                LOSE_QUEST_BOOK_ON_DEATH_KEY,
                LOSE_QUEST_BOOK_ON_DEATH_DEFAULT,
                LOSE_QUEST_BOOK_ON_DEATH_COMMENT)
            .getBoolean(LOSE_QUEST_BOOK_ON_DEATH_DEFAULT);
