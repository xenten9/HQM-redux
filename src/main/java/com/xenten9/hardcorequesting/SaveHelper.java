...
public final class SaveHelper {
    ...
    public static void render(GuiQuestBook gui, int mX, int mY) {
        ...
        gui.drawRect(
            X + CHANGE_X,
            Y + CHANGE_Y,
            CHANGE_SRC_X + indexX * CHANGE_SIZE,
            SRC_Y + indexY * CHANGE_SIZE,
            CHANGE_SIZE,
            CHANGE_SIZE);
        ...
        if (isLarge) {
            if (total == 0) {
                ...
                if (saveTime == 0) {
                    gui.drawString(
                        Translator.translate("hqm.editType.neverSaved"),
                        X + START_X,
                        Y + START_Y,
                        0.7F,
                        0x404040);
                    ...
                    gui.drawString(
                        formatTime((int) ((Minecraft.getSystemTime() - saveTime) / 60000)),
                        X + START_X,
                        Y + START_Y,
                        0.7F,
                        0x404040);
                ...
                gui.drawString(
                    Translator.translate("hqm.editType.unsaved", total),
                    X + START_X,
                    Y + START_Y + 2 * FONT_HEIGHT,
                    0.7F,
                    0x404040);
                int others = total;
                for (int i = 0; i < LISTED_TYPES; i++) {
                    ...
                    gui.drawString(
                        element.type.translate(element.count),
                        X + START_X + INDENT,
                        Y + START_Y + (i + 3) * FONT_HEIGHT,
                        0.7F,
                        0x404040);
                ...
                if (others > 0) {
                    gui.drawString(
                        Translator.translate("hqm.editType.other", others),
                        X + START_X + INDENT,
                        Y + START_Y + (LISTED_TYPES + 3) * FONT_HEIGHT,
                        0.7F,
                        0x404040);
                }
            }
