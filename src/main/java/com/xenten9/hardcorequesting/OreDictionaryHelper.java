...
public class OreDictionaryHelper {
    ...
    public static boolean match(ItemStack itemStack1, ItemStack itemStack2) {
        ...
        for (int id1 : ids1) for (int id2 : ids2) if (id1 == id2) return true;
        return false;
    }
    ...
    public static List<ItemStack> getPermutationsList(ItemStack itemStack) {
        ...
        if (permutations.size() == 0) permutations.add(itemStack);
    ...
    public static String[] getOreNames(ItemStack itemStack) {
        ...
        for (int id : OreDictionary.getOreIDs(itemStack)) names.add(OreDictionary.getOreName(id));
    ...
    private static List<ItemStack> stripWildcardDamage(List<ItemStack> list) {
        ...
        for (ItemStack itemStack : list) result.add(
            new ItemStack(
                itemStack.getItem(),
                itemStack.stackSize,
                itemStack.getItemDamage() == OreDictionary.WILDCARD_VALUE ? 0 : itemStack.getItemDamage()));
        return result;
    }
}
