...
import com.xenten9.hardcorequesting.blocks.BlockInfo;
import com.xenten9.hardcorequesting.quests.Quest;
import com.xenten9.hardcorequesting.quests.QuestDataTaskItems;
import com.xenten9.hardcorequesting.quests.QuestTask;
import com.xenten9.hardcorequesting.quests.QuestTaskItemsConsume;
...
public class TileEntityBarrel extends TileEntity implements IInventory, IFluidHandler {
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        ...
        if (doFill) {
            ...
            if (task != null && task instanceof QuestTaskItemsConsume) {
                if (((QuestTaskItemsConsume) task)
                    .increaseFluid(resource.copy(), (QuestDataTaskItems) task.getData(playerName), playerName)
                    && modifiedSyncTimer <= 0) {
    ...
    public void setInventorySlotContents(int i, ItemStack itemstack) {
        ...
        if (task != null && task instanceof QuestTaskItemsConsume) {
            if (((QuestTaskItemsConsume) task)
                .increaseItems(new ItemStack[] { itemstack }, (QuestDataTaskItems) task.getData(playerName), playerName)
                && modifiedSyncTimer <= 0) {
                modifiedSyncTimer = SYNC_TIME;
            }
        }
    ...
    public QuestTask getCurrentTask() {
        if (playerName != null && selectedQuest >= 0 && selectedQuest < Quest.size()) {
            ...
            if (quest != null && selectedTask >= 0
                && selectedTask < quest.getTasks()
                    .size()) {
