...
import com.xenten9.hardcorequesting.client.interfaces.GuiEditMenuPortal;
...
public class TileEntityPortal extends TileEntity implements IBlockSync {
    ...
    public boolean hasTexture(EntityPlayer player) {
        return players.contains(
            player.getGameProfile()
                .getName()) ? completedTexture : uncompletedTexture;
    }

    public boolean hasCollision(EntityPlayer player) {
        return players.contains(
            player.getGameProfile()
                .getName()) ? completedCollision : uncompletedCollision;
    }
    ...
    private void keepClientDataUpdated() {
        ...
        if (distance > Math.pow(PacketHandler.BLOCK_UPDATE_RANGE, 2)) {
            ...
            && distance < Math.pow(PacketHandler.BLOCK_UPDATE_RANGE - PacketHandler.BLOCK_UPDATE_BUFFER_DISTANCE, 2)) {
                hasUpdatedData = true;
