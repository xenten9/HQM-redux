package com.xenten9.hardcorequesting.bag;
...
import com.xenten9.hardcorequesting.quests.ItemPrecision;
...
public class Group {
    ...
    public Group() {
        ...
        if (GroupTier.getTiers()
            .size() > 1) {
            this.tier = GroupTier.getTiers()
                .get(1);
        } else {
            this.tier = GroupTier.getTiers()
                .get(0);
    ...
    public void open(EntityPlayer player) {
        if (limit > 0) {
            GroupData data = QuestingData.getQuestingData(player)
    ...
    public static void readAll(DataReader dr, FileVersion version) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            GroupTier tier = GroupTier.getTiers()
                .get(dr.readData(DataBitHelper.TIER_COUNT));
    ...
    public boolean equals(Object obj) {
        if (obj instanceof Group) {
            if (Objects.equals(name, ((Group) obj).name) && limit == ((Group) obj).limit
                && items.size() == ((Group) obj).items.size()) {
    ...
    public static void drawOverview(GuiQuestBook gui, ScrollBar tierScroll, ScrollBar groupScroll, int x, int y) {
        ...
        for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_TIERS, tiers.size()); i++) {
            ...
            boolean inBounds = gui
                .inBounds(GuiQuestBook.TIERS_X, yPos, gui.getStringWidth(str), GuiQuestBook.TEXT_HEIGHT, x, y);
            int color = groupTier.getColor()
                .getHexColor();
            if (inBounds) {
                color &= 0xFFFFFF;
                color |= 0xBB << 24;
            ...
            for (int j = 0; j < BagTier.values().length; j++) {
                ...
                gui.drawCenteredString(
                    bagTier.getColor()
                        .toString() + groupTier.getWeights()[j],
                    GuiQuestBook.TIERS_X + GuiQuestBook.TIERS_SECOND_LINE_X + j * GuiQuestBook.WEIGHT_SPACING,
                    yPos + GuiQuestBook.TIERS_SECOND_LINE_Y,
                    0.7F,
                    GuiQuestBook.WEIGHT_SPACING,
                    0,
                    0x404040);
            }
        }
        ...
        start = groupScroll.isVisible(gui)
            ? Math.round((groups.size() - GuiQuestBook.VISIBLE_GROUPS) * groupScroll.getScroll())
            : 0;
        for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_GROUPS, groups.size()); i++) {
            ...
            boolean inBounds = gui
                .inBounds(GuiQuestBook.GROUPS_X, yPos, gui.getStringWidth(str), GuiQuestBook.TEXT_HEIGHT, x, y);
            int color = group.getTier()
                .getColor()
                .getHexColor();
    ...
    public void draw(GuiQuestBook gui, int x, int y) {
        gui.drawString(
            this.getName(),
            GuiQuestBook.GROUPS_X,
            GuiQuestBook.GROUPS_Y,
            this.getTier()
                .getColor()
                .getHexColor());
        ...
        for (int i = 0; i < Math.min(DataBitHelper.GROUP_ITEMS.getMaximum(), items.size() + 1); i++) {
            ...
            int xPos = (i % GuiQuestBook.ITEMS_PER_LINE) * GuiQuestBook.GROUP_ITEMS_SPACING
                + GuiQuestBook.GROUP_ITEMS_X;
            int yPos = (i / GuiQuestBook.ITEMS_PER_LINE) * GuiQuestBook.GROUP_ITEMS_SPACING
                + GuiQuestBook.GROUP_ITEMS_Y;
    ...
    public static void mouseClickedOverview(GuiQuestBook gui, ScrollBar groupScroll, int x, int y) {
        ...
        int start = groupScroll.isVisible(gui)
            ? Math.round((groups.size() - GuiQuestBook.VISIBLE_GROUPS) * groupScroll.getScroll())
            : 0;
        for (int i = start; i < Math.min(start + GuiQuestBook.VISIBLE_GROUPS, groups.size()); i++) {
            Group group = groups.get(i);
            ...
            if (gui.inBounds(
                GuiQuestBook.GROUPS_X,
                posY,
                gui.getStringWidth(group.getName()),
                ...
                switch (gui.getCurrentMode()) {
                    ...
                    case NORMAL:
                        ...
                        gui.getTextBoxGroupAmount()
                            .setTextAndCursor(
                                gui,
                                String.valueOf(
                                    GuiQuestBook.getSelectedGroup()
                                        .getLimit()));
                        break;
                    case RENAME:
