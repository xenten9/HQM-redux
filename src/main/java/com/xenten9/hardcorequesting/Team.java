...
import com.xenten9.hardcorequesting.quests.QuestData;
...
public class Team {
    ...
    public void resetProgress(Quest quest) {
        questData.set(
            quest.getId(),
            Quest.getQuest(quest.getId())
                .createData(getPlayerCount()));
    }
    ...
    public void receiveAndSyncReputation(Quest quest, List<ReputationReward> reputationList) {
        for (ReputationReward reputationReward : reputationList) {
            setReputation(
                ...
                getReputation(reputationReward.getReward()) + reputationReward.getValue());
        }
    ...
    public int getSharedLives() {
        ...
        for (Team.PlayerEntry entry : getPlayers()) {
            if (entry.inTeam) {
                lives += QuestingData.getQuestingData(entry.getName())
                    .getRawLives();
            }
        }
        return lives;
        ...
        for (PlayerEntry player : players) {
            if (player.inTeam) {
                if (player.getName()
                    ...
                    leaveTeam.getPlayers()
                        .get(0)
                        .setBookOpen(player.bookOpen);
    ...
    private void refreshTeamData(PlayerEntry entry, UpdateType type) {
        ...
        switch (type) {
            ...
            case ONLY_INVITES:
                ...
                team = QuestingData.getQuestingData(entry.getName())
                    .getTeam();
                valid = !entry.isInTeam() && team.getEntry(entry.getName())
                    .shouldRefreshData();
                break;
            case ONLY_MEMBERS:
    ...
    public void refreshData() {
        for (PlayerEntry entry : getPlayers()) {
            if (entry.shouldRefreshData()) {
                QuestingData.getQuestingData(entry.getName())
                    .refreshClientData(entry.getName());
            }
        }
    }
    ...
    public void clearProgress() {
        ...
        for (int i = 0; i < Quest.size(); i++) {
            if (Quest.getQuest(i) != null && questData.get(i) != null) {
                Quest.getQuest(i)
                    .preRead(playerCount, questData.get(i));
            }
        }
        refreshData();
    }
    ...
    public void deleteTeam() {
        for (int i = players.size() - 1; i >= 0; i--) {
            ...
            QuestingData.getQuestingData(player.getName())
                .getTeam()
                .refreshTeamData(player, UpdateType.ONLY_MEMBERS);
        }
        ...
        for (String username : FMLCommonHandler.instance()
            .getMinecraftServerInstance()
            .getConfigurationManager()
            .getAllUsernames()) {
            Team team = QuestingData.getQuestingData(username)
                .getTeam();
        ...
        switch (action) {
            ...
            case INVITE:
                if (!team.isSingle() && team.isOwner(playerName)) {
                    ...
                    if (!QuestingData.getQuestingData(entry.getName())
                        .getTeam()
                        .isSingle()) {
            ...
            case ACCEPT:
                if (team.isSingle()) {
                    ...
                    if (acceptId >= 0 && acceptId < QuestingData.getTeams()
                        .size()) {
                        Team inviteTeam = QuestingData.getTeams()
                            .get(acceptId);
                        int id = 0;
                            ...
                            if (entry.inTeam) {
                                ...
                                entry.setBookOpen(
                                    team.getPlayers()
                                        .get(0).bookOpen);
                                entry.inTeam = true;
                                QuestingData.getQuestingData(entry.name)
                                    .setTeam(inviteTeam);
                                ...
                                for (int i = 0; i < inviteTeam.questData.size(); i++) {
                                    ...
                                    if (questData != null && Quest.getQuest(i) != null) {
                                        Quest.getQuest(i)
                                            .mergeProgress(playerName, questData, joinData);
                                    }
                                }
            ...
            case DECLINE:
                if (team.isSingle()) {
                    ...
                    if (declineId >= 0 && declineId < QuestingData.getTeams()
                        .size()) {
                        Team inviteTeam = QuestingData.getTeams()
                            .get(declineId);
                        inviteTeam.getPlayers()
                            .remove(new PlayerEntry(playerName, false, false));
            ...
            case KICK:
                if (!team.isSingle() && team.isOwner(playerName)) {
                    ...
                    if (!entryToRemove.isOwner()) {
                        ...
                        QuestingData.getQuestingData(playerToRemove)
                            .getTeam()
                            .refreshTeamData(UpdateType.ONLY_MEMBERS);
                    }
                }
            ...
            case NEXT_LIFE_SETTING:
                if (!team.isSingle() && team.isOwner(playerName)) {
                    team.lifeSetting = LifeSetting.values()[(team.lifeSetting.ordinal() + 1)
                        % LifeSetting.values().length];
                    team.refreshTeamData(UpdateType.ALL);
                }
                break;
    ...
    private static void declineAll(String playerName) {
        for (Team team : QuestingData.getTeams()) {
            for (Iterator<PlayerEntry> iterator = team.getPlayers()
                .iterator(); iterator.hasNext();) {
                PlayerEntry playerEntry = iterator.next();
                if (!playerEntry.isInTeam() && playerEntry.getName()
                    .equals(playerName)) {
                    iterator.remove();
                    team.refreshTeamData(UpdateType.ONLY_OWNER);
                    break;
            ...
            for (int i = 0; i < questData.size(); i++) {
                if (questData.get(i) != null) {
                    Quest.getQuest(i)
                        .preRead(playerCount, questData.get(i));
                    Quest.getQuest(i)
                        .read(dr, questData.get(i), version, true);
                }
            }
    ...
    public void postRead(QuestingData data, FileVersion version) {
        for (int i = 0; i < questData.size(); i++) {
            if (Quest.getQuest(i) != null && questData.get(i) != null) {
                Quest.getQuest(i)
                    .postRead(data, questData.get(i), version);
            }
        }
    }
